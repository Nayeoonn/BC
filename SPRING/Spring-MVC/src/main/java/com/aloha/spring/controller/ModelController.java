...
//		model.addAttribute(person); // 변수명을 생략하면 객체명과 일치하는 이름으로 접근할 수 있다.
	...
	/**
	 * 요청 경로 : /model/person2?name=김조은&age=20 * 요청 파라미터의 변수명들을 객체의 필드(멤버변수)로 자동으로
	 * 매핑해준다.
	 * 
	 * @param person
	 * @return
...
//		 model.addAttribute("person", person);
	...
	/**
	 * 요청 경로 : /model/person method : POST
	 * 
	 * @param person
	 * @return
	 */
	@RequestMapping(value = "/model/person", method = RequestMethod.POST)
	public String personPost(@RequestParam("name") String personName, @RequestParam("age") int personAge, Model model) {
