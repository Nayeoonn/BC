-- Active: 1709190582789@@127.0.0.1@1521@orcl@HR
...
-- 5. 
-- 테이블 EMPLOYEES의 JOB_ID를 중복된 데이터를 제거하고
-- 조회하는 SQL문을 작성하시오.
...
-- 16.
-- 테이블 EMPLOYEES 의 FIRST_NAME 이 
-- 5글자인 사원의 모든 컬럼을 조회하는 SQL 문을 작성하시오.
...
SELECT *
FROM employees
WHERE hire_date >= '04-JAN-01';    
-- * 데이터의 월 값이 숫자면 --> '04/01/01'
-- * 데이터의 월 값이 문자면 --> '04/JAN/01'
...
-- 21. 
-- 12.45, -12.45 보다 
-- 크거나 같은 정수 중 제일 작은 수를 계산하는 SQL 문을 각각 작성하시오.
-- * dual ?
...
-- 125.67을 소수점 일의 자리에서 반올림하시오
SELECT ROUND(125.67, -1) 
FROM dual;

-- 125.67을 소수점 십의 자리에서 반올림하시오
...
-- ex) 'ALOHACLASS'
-- 해당 문자열에서 첫글자부터 찾아서, 2번쨰 A의 위치를 구하시오.
...
-- RPAD(문자열, 칸의 수, 채울 문자)
-- : 문자열에 지정한 칸을 확보하고, 오른쪽에 특정 문자로 채운
-- 'ALOHACLASS'
...
-- : 특정 데이터를 문자열 형식으로 반환하는 함수
SELECT first_name AS 이름
     , TO_CHAR(hire_date, 'YYYY_MM_DD (DY) HH:MI:SS') AS 입사일자
FROM employees;
...
-- 월말 : LASTDAY(날짜)
/*
     날짜 데이터 : XXXXXX.YYYYYYYYY
     1970년1월1일 00시00분00초00ms
     지난 일자를 정수로 계산, 시간 정보는 소수부분으로 계산
     TRUCN(XXXXXX.YYYYYYYYY) --> XXXXXX
     정수 부분인 년,월,일 만 남는다.
     마찬가지로, 월 단위 기준으로 절삭하면 월 초를 구할 수 있다.
*/
...
/*
  * 실행 순서
  ...
  2. 조건에 맞는 데이터 선택
  3. 그룹기준 지정
  4. 그룹별로 그룹조건에 맞는 데이터 선택
  5. 조회할 컬럼 선택
  6. 조회한 결과를 정렬기준에 따라 정렬 
*/
-- NVL(값, 대체할 값) : 해당 값이 NULL이면 지정된 값으로 변환하는 함수
-- DISTINCT : 중복 없이 조회

-- ORDER BY의 정렬 기준 컬럼은, SELECT에서 선택한 컬럼만 사용 가능
SELECT DISTINCT NVL(commission_pct,0) "커미션(%)"
FROM employees
ORDER BY NVL(COMMISSION_PCT, 0) DESC;

-- 조회한 컬럼의 별칭으로 ORDER BY 절에서 사용가능
SELECT DISTINCT NVL(commission_pct,0) "커미션(%)"
FROM employees
ORDER BY "커미션(%)" DESC;
...
-- 급여, 커미션, 최종급여를 조회하시오. 최종급여 기준으로 내림차순 정렬하시오.
-- * 최종급여 = 급여 + (급여 * 커미션)
-- * NVL2(값, NULL 아닐 때 값, NULL일 떄 값)
SELECT first_name 이름
     , salary 급여
     , commission_pct
...
-- 46.
-- DEPARTMENTS 테이블을 참조하여, 사원의 이름과 부서명 출력
...
/*
    CREATE TABLE 테이블명 (
        컬럼명1 타입     [NOT NULL/NULL] [DEFAULT 기본값] [제약조건],
        컬럼명2 타입     [NOT NULL/NULL] [DEFAULT 기본값] [제약조건],
        컬럼명3 타입     [NOT NULL/NULL] [DEFAULT 기본값] [제약조건],
        ...
    );
*/
CREATE TABLE MS_STUDENT (
    ...
    ,UP_DATE   DATE    DEFAULT sysdate NOT NULL
...
COMMENT ON COLUMN MS_STUDENT.UP_DATE IS '수정일자';
...
ADD CONSTRAINT MS_STUDNET_GENDER_CHECK
