-- Active: 1709601471157@@127.0.0.1@1521@orcl@JOEUN
-- 70.
...
imp userid=system/123456 file=c:\QWERT\ORACLE\joeun.dmp fromuser=joeun touser=joeun


-- 71.
-- joeun 계정의 데이터를 덤프파일로 export 하기
exp userid=joeun/123456 file=c:\QWERT\ORACLE\community.dmp log=c:\QWERT\ORACLE\community.log
...
-- 76.
-- 테이블 MS_FILE의 FILE_NAME 속성에서 확장자를 추출하여
-- EXT 속성에 UPDATE 하는 SQL문을 작성하시오.
...
-- * DELETE - 데이터 조작어(DML)
--  - 한 행 단위로 데이터를 삭제한다.
--  - WHERE 조건절을 기준으로 일부만 삭제도 가능
--  - COMMIT, ROLLBACK을 이용하여 변경사항을 적용한거나 되돌릴 수 있음

-- * TRUNCATE - 데어터 정이의(DDL)
--  - 모든 행을 삭제한다.
--  - 삭제된 데이터를 되돌릴 수 없음
...
FROM EMPLOYEE E
      LEFT JOIN DEPARTMENT D ON E.DEPT_CODE = D.DEPT_ID
      JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
WHERE E.EMP_ID NOT IN ( -- NOT만 붙이면 일반 사원이 됨
                        SELECT DISTINCT MANAGER_ID
                        FROM EMPLOYEE
                        WHERE MANAGER_ID IS NOT NULL
                  )
