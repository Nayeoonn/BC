...
/* (예외처리)
 * - 입력시 자료형이 입력 메소드와 일치하지 않아서 발생하는 예외상황
 *  ex) nextInt에 대하여 정수를 입력하지 않고 다른 자료형(문자 등) 입력한 경우 
 */
public class InputMismatch {
	...
	public static void main(String[] args) {
		...
		do {
			...
			// 예외 메시지 : jaca.util.InputMismatchException
			// 예외 상황 : 숫자 입력 메소드에 문자를 입력한 경우 등
			try {
				menuNo = sc.nextInt();		// 예외 발생 가능성이 있ㄴ느 문장
				
			} catch (InputMismatchException e) {
				// 숫자를 입력하지 않았으면, 다시 반복해서 입력하도록
				sc.next();				// 입력 스트림에서 남아있는 엔터 제거
