...
public class BoardDAO extends JDBConnection{
	...
	public List<Board> list(){
		...
		try {
			...
			while(rs.next()) {		// next() : 실행 결과의 다음 데이터로 이동
				...
				board.setUpDate(rs.getTimestamp("up_date"));
	...
	public Board select(int no){
		...
		try {
			...
			psmt.setInt(1, no);  	// ?(1) 물을표 1번에 글번호(no)를 넣어준다
	...
	public int insert(Board board) {
		...
		try {
			...
			psmt.setString(2, board.getWriter());	// 2번 ? 에 작성자 매
			psmt.setString(3, board.getContent());	// 3번 ? 에 내용을 매핑
			
			result = psmt.executeUpdate();	// SQL 실행 요청, 적용시 데이터 개수를 받아온다
											// 게시글 1개 적용 성공시, result : 1
											//			   실패시, result : 0
			// executeUpdate()
